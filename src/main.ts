...
app.appendChild(
  details(
    ...
    p(
      'That BBS98 falls to collusion IS proved on the page, in the strong sense: the recovered scalar is compared against the real one and, independently, against the public key derived from it. That AFGH does not fall the same way is shown only for the attacks demonstrated here. The actual guarantee is AFGH’s Theorem 3.1, which reduces master-secret security to the discrete logarithm problem — a reduction no browser demo can reproduce.'
    ),
    p(
    ...
    p(
      'The AFGH scheme is written in its original papers for a symmetric pairing. BLS12-381 is Type-3, so the source-group roles are split across G1 and G2. That split is forced by the pairing’s signature rather than chosen, and the Scheme Card on the Relay tab spells it out — but it is an adaptation, and the security arguments were written for the symmetric setting.'
    )
  )
);
